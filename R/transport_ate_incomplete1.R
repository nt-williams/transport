transport_ate_incomplete1 <- function(transport_Npsem, learner, family, folds = 20) {
    ...
    f_VZ <- vector("numeric", nrow(transport_Npsem$data))
    ...
    for (i in 1:folds) {
        ...
        # E(f(V) | Z)
        fit_fVZ <- train(Z[t, , drop = FALSE],
                         f_Vt, "gaussian", learner, 10)

        f_VZ[v] <- predict_from_fit(fit_fVZ, Z[v, , drop = FALSE])
    }

    theta_init <- mean(f_VZ[s == 0])
