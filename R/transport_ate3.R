#' Transport Estimator for the Average Treatment Effect using Known Effect/Transport Modifiers
#'
#' TODO
#'
...
#' @param transport_modifiers [\code{character}]\cr
...
#'
#' @seealso [.transport_ate3_control()].
#'
#' @return
#' @export
#'
#' @examples
transport_ate3 <- function(data, trt, outcome, source, covar,
                           effect_modifiers, transport_modifiers,
                           cens = NULL,
    ...
    checkmate::assert_character(transport_modifiers)
    checkmate::assert_character(cens, len = 1, null.ok = TRUE)
    checkmate::assert_character(id, len = 1, null.ok = TRUE)
    checkmate::assertSubset(c(effect_modifiers, transport_modifiers), covar)
    ...
    # Fit the probability of being in the target population: P(S=1 | Z)
    fit_S <- crossfit(data = data,
                      target = source,
                      covar = transport_modifiers,
                      folds = folds,
                      assignments = NULL,
                      outcome_type = "binomial",
    ...
    # Fit the outcome regression: E(Y|S=1,A,W:{W, V, Z})
    ...
    # E(f(V) | Z)
    fit_fVZ <- crossfit(data = cbind(data, foovar = fit_fV$pred[, 1]),
                        target = "foovar",
                        covar = transport_modifiers,
                        folds = folds,
                        assignments = NULL,
    ...
    eif <- eif_transport_ate3(data, trt, outcome, source,
                              fit_pi$pred, fit_S$pred, fit_m$pred,
                              fit_fV$pred, fit_fVZ$pred)
    ...
    class(out) <- c("transport", "transport_ate3")
    out
}

print.transport_ate3 <- function() {
