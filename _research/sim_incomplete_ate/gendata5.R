...
safe_sim <- possibly(function(n) {
    ...
    lambda <- transport_ate(Np, c("SL.glm", "SL.mean"), "gaussian", folds)

    Np <- transport_Npsem$new(dat, c("W1"), V = NULL, Z = "A", S = "S", Y = "Y")
    theta <- transport_ate_incomplete(Np, c("SL.glm", "SL.mean"), "gaussian", folds)
...
saveRDS(out, "_research/sim_incomplete_ate/results/dgp1-cf.rds")
