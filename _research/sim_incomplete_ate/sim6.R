...
source("_research/sim_incomplete_ate/dgp6.R")
...
sim <- possibly(function(n) {
    ...
    Np <- transport_Npsem$new(dat, c(w, v, z), A = "A", S = "S", Y = "Y")
    clambda2 <- transport_ate_incomplete_sans_Z(Np, c("SL.glm", "SL.glm.interaction", "SL.mean", "SL.ranger", "SL.earth"),
                                                "gaussian", F, folds)

    # Np <- transport_Npsem$new(dat, c(w, v, z), A = "A", S = "S", Y = "Y")
    # clambda1 <- transport_ate_incomplete_sans_V(Np, c("SL.glm", "SL.glm.interaction", "SL.mean", "SL.ranger", "SL.earth"),
    #                                             "gaussian", "adaptive-lasso", folds)
    #
    # clambda2 <- transport_ate_incomplete_sans_V(Np, c("SL.glm", "SL.glm.interaction", "SL.mean", "SL.ranger", "SL.earth"),
    #                                             "gaussian", "adaptive-lasso-sl", folds)
    #
    # clambda3 <- transport_ate_incomplete_sans_V(Np, c("SL.glm", "SL.glm.interaction", "SL.mean", "SL.ranger", "SL.earth"),
    #                                             "gaussian", "sl", folds)

    data.frame(estimator = c("lambda", "theta", "theta1", "theta2","clambda1", "clambda2"),
               order = 1:6,
               psi = c(lambda$theta, theta$theta, theta1$theta, theta2$theta,clambda1$theta, clambda2$theta),
               var = c(lambda$var, theta$var, theta1$var, theta2$var, clambda1$var, clambda2$var),
               covered = map_lgl(list(lambda, theta, theta1, theta2, clambda1, clambda2), covered))
}, NULL)
...
saveRDS(res, glue("_research/sim_incomplete_ate/results/raw/dgp6_{crossfit}_{id}.rds"))
