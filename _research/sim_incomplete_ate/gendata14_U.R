...
safe_sim <- possibly(function(n) {
    ...
    data.frame(estimator = c("gamma2", "gamma2_ipw", "gamma3", "gamma3_ipw"),
               order = 1:4,
               psi = c(gamma2$theta, gamma2$ipw, gamma3$theta, gamma3$ipw),
               var = c(gamma2$var, gamma2$ipw_var, gamma3$var, gamma3$ipw_var),
               covered = c(covered(gamma2)[1], covered(gamma2)[2], covered(gamma3)[1], covered(gamma3)[2]))
}, NULL)
